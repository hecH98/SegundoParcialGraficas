...
function genEllipseData(a = 1., b=0.7, n = 100)
{
	vertices = [];
	indices = [];
	var x, y, z;
...
function genCycloidData(r = 0.17, n = 100)
{
	vertices = [];
	indices = [];
	var x, y, z;
...
function genHypocloidData(r = 0.68, n = 100)
{
	vertices = [];
	indices = [];
	var x, y, z;
	...
	for(var i = 0; i <= n; i++)
	{
		x = r * Math.pow(Math.cos(theta),3);
		y = r * Math.pow(Math.sin(theta), 3);
		vertices.push(x);
		vertices.push(y);
		vertices.push(z);
		theta = theta + deltaTheta;
